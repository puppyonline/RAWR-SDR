      ...
      <div className="card p-5">
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-xs text-muted font-mono mt-0.5">87.5 &ndash; 108.0 MHz &middot; NRSC-5</p>
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {Object.entries(metadata).map(([key, val]) => (
            <div key={key} className="card-inner p-3">
              <p className="text-[10px] text-muted uppercase tracking-wide mb-1">{key}</p>
              <p className="text-sm font-medium text-secondary truncate">{val}</p>
            </div>
          ))}
        </div>
      </div>
          ...
          {hdPresets.map((p) => (
            ...
            >
              <div className="flex items-center justify-between mb-0.5">
                <span className="text-[10px] text-muted truncate">{p.label}</span>
