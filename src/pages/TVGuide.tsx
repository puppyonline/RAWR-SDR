...
function TVGuide() {
  ...
  // Show 4 hours of programming
  const startTime = now - (now % 1800); // round to nearest 30 min
    ...
    <div className="space-y-4">
        ...
        <div ref={gridRef} className="max-h-[calc(100vh-220px)] overflow-y-auto">
          {guide.map((ch) => (
            <div key={ch.GuideNumber} className="flex border-b border-bg-border last:border-0 hover:bg-bg-hover/30 transition-colors">
              {/* Channel label */}
              <div className="w-32 md:w-40 shrink-0 p-2 border-r border-bg-border flex items-center gap-2">
      ...
      {selectedProgram && (
        <div className="card p-5">
          ...
          <div className="mt-4">
            <Link to="/tv" className="btn-brand btn-sm">Watch Now</Link>
          </div>
        </div>
      )}
    </div>
